"""Массовый парсинг всех вузов Кыргызстана через Groq.

ЗАПУСК:
    python parse_all.py                    # парсит все 61 вузов
    python parse_all.py --limit 5          # только первые 5 (для теста)
    python parse_all.py --no-reviews       # без отзывов (быстрее)
    python parse_all.py --skip-existing    # пропустить уже спарсенные

ОЦЕНКА ВРЕМЕНИ (Groq):
- Каждый вуз: ~5-10 секунд (Groq очень быстрый)
- 61 вуз с отзывами: ~7-12 минут
- Лимиты Groq: 30 запросов/минуту, 14,400/день — нам нужно всего 122 запроса

ВАЖНО: не закрывай терминал во время парсинга, иначе процесс прервётся.
Уже спарсенные вузы сохранятся в БД (--skip-existing подхватит их при перезапуске).
"""
import argparse
import time
